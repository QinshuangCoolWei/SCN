function solution = concave_minimization_bf(par_w,option,out_filename)
% bruteforce with all possible alpha/beta
% w = 1;      % weight for time
...
%         p_slice = reshape(diag(num_phi*max(c_prod,[],'all')*ones(N_V*N_W,1)),N_V*N_W,1,N_V*N_W); % create a 3d slice
...
%         y_slice = reshape(diag(num_phi*max(c_trans,[],'all')*ones(N_E*N_W,1)),N_E*N_W,1,N_E*N_W); % create a 3d slice
...
%         I_slice = reshape(diag(num_phi*(sum(I_0,'all')+max(c_prod,[],'all')*ones(N_V*N_W,1))),N_V*N_W,1,N_V*N_W); % create a 3d slice
...
% find all possible binary matrices first (alpha = N_E x 1, beta = N_V x N_K)

num_elements = N_E;
num_alpha = 2^num_elements;
alpha_vec = dec2bin(0:num_alpha-1) - '0';   % Generate all binary combinations
% Reshape each binary vector into an m x n matrix and store in a cell array
alpha_list = cell(num_alpha, 1);
for i = 1:num_alpha
    alpha_list{i} = reshape(alpha_vec(i,:), N_E, 1);
end
% 
% % Example: display the first binary matrix
% disp(alpha_list{1})

%     beta(:,ind_raw>0) == 0,...
%     beta(ind_no_prod,:) == 0,...
mask = true(N_V, N_K);
mask(par_w.ind_no_prod, :) = false;
mask(:, par_w.ind_raw>0) = false;
num_free = nnz(mask);
num_beta = 2^num_free;

% num_elements = find(par_w.ind_raw==0)*find(par_w.ind_no_prod);
beta_vec = dec2bin(0:num_beta-1) - '0';   % Generate all binary combinations
% Reshape each binary vector into an m x n matrix and store in a cell array
beta_list = cell(num_beta, 1);
for i = 1:num_beta
    mat = zeros(N_V, N_K);         % initialize full-zero matrix
    mat(mask) = beta_vec(i,:);  % assign only free positions
    beta_list{i} = mat;
end


solution_all = cell(num_alpha*num_beta,1);
obj_all = zeros(num_alpha*num_beta,1);
count_ab = 0;
for i_a = 1:num_alpha
    for i_b = 1:num_beta
        count_ab = count_ab+1;
        ...
        %         beta = zeros(3,3);
        %         beta(2:end,2:end) = 1;
        alpha = alpha_list{i_a};
        beta = beta_list{i_b};
        alpha = [1;1];
        beta = zeros(3,3);
        beta(2,2) = 1;
        beta(3,3) = 1;
        S.tau = tau_0+1;        % renew all S
        S.p = p_3d;
        S.y = y_3d;
        S.x = x_3d;
        S.I = I_3d;
...
%             UB-LB
            ...
            elseif option == 0  % plan with given beta (no w)
                ...
                tstart = tic;
                sol = model_solving_cont(par_w,S,phi_in,Mask,alpha,beta);
                telapsed = toc(tstart);
            end
            % append
            p_temp = sol.p;
...
%             oldSol.nu_4 = sol.nu_4;
        ...
        solution = sol_min.obj;
        solution_all{count_ab} = solution;
        obj_all(count_ab) = UB;
    end
end
save(out_filename)
