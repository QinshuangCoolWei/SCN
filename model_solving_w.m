function solution = model_solving_w(par,S,phi_in,Mask, M_1,M_2, scale,init_guess)
% solve the MIP for each initial point
format long;
digits(100)
% parameters
% eps = 1e-5;
...
f_prod = par.f_prod*scale;
f_trans = par.f_trans*scale;
rho = par.rho*scale;
tau = S.tau;
...
alpha = binvar(N_E,1,'full');
beta = binvar(N_V,N_K,'full');
% alpha = sdpvar(N_E,1,'full');
% beta = sdpvar(N_V,N_K,'full');

% zeta = binvar(N_V,N_C,'full');
% E_penalty = sdpvar(1,1,'full');
% f_trans_cost = sdpvar(1,1,'full');
% f_prod_cost = sdpvar(1,1,'full');
...
%     zeta*ones(N_C,1) <= 1,...
    ...
    sum(S.alpha.*repmat(reshape(mu,1,1,tau),N_E,1),3) == alpha,...
    sum(S.beta.*repmat(reshape(mu,1,1,tau),N_V,N_K),3) == beta,...
    mu.*nu_3==0,...
%     nu_3 <= M_1*nu_4,...
%     mu <= M_2*(1-nu_4),...
    mu >= 0,...
...
%                 phi_in(t) - nu_1 - sum(nu_2_p.*S.p(:,:,t),"all")- sum(nu_2_y.*S.y(:,:,t),"all")+ nu_3(t) == 0,...
        ...
        +nu_2_I.*S.I(:,:,t)+nu_2_Delta.*S.Delta(:,:,t),"all")-sum(nu_2_beta.*S.beta(:,:,t),"all") ...
        - sum(nu_2_y.*S.y(:,:,t),"all") - sum(nu_2_alpha.*S.alpha(:,:,t),"all") + nu_3(t) == 0
        ];
